...
library(stats) # don't need to install stats, is native package. Do need
# to make sure it is loaded though, so still run the library(stats)
...
DFMData_numeric(dfm1.180201.at, "at")
DFMData_numeric(dfm2.180201.at, "at")
DFMData_numeric(dfm3.180201.at, "at")
DFMData_numeric(dfm4.180201.at, "at")
DFMData_numeric(dfm5.180201.at, "at")
...
DFMData_numeric(dfm6.180201.at, "at")
DFMData_numeric(dfm7.180201.at, "at")
DFMData_numeric(dfm8.180201.at, "at")
DFMData_numeric(dfm9.180201.at, "at")
DFMData_numeric(dfm10.180201.at, "at")
...
#### If you get an error related to memory size, first try to increase the memory available
#### to Rstudio by using the function memory.limit() where you put the desired memory size
#### in parenthesese. Just max out the possible memory by throwing in some ridiculously large
#### number in there, like 99999999999, then try running again. Chances are problem solved

# Ex: memory.limit(9999999999)
...
## Before plotting, will possibly need to truncate data if combining data
## across multiple experiments. To do this, you will need to use a sequence
## of two functions, 1. subset.data() and 2. genotype.means()
...
# Sample function to randomize genotype placements
...
### Plotting Script for Period Length as done in Yao & Shafer 2014 paper ###
# plotting function for boxplots of genotype periods

test.isoSG <- subset(to.data, to.data$Genotype=="iso/SGG(hypo)")
test.isoL <- subset(to.data, to.data$Genotype == "iso/DBT(L)")
test.toSG <- subset(to.data, to.data$Genotype == "to-gal4>SGG(hypo)")
test.toL <- subset(to.data, to.data$Genotype == "to-gal4>DBT(L)")
test.pdfSG <- subset(to.data, to.data$Genotype == "pdf-gal4>SGG(hypo)")
test.pdfL <- subset(to.data, to.data$Genotype == "pdf-gal4>DBT(L)")
test.pdfiso <- subset(to.data, to.data$Genotype == "pdf-gal4/iso")
test.toiso <- subset(to.data, to.data$Genotype == "to-gal4/iso")

to.plot.data <- rbind(test.toiso, test.pdfiso, test.pdfSG, test.pdfL, test.toSG, test.toL)

p.to <- ggplot(to.plot.data, aes(x=Genotype, y = Period)) +
  #geom_boxplot(colour = "white") +
  #geom_dotplot(binaxis = "y", stackdir = "center", binwidth = 0.20, dotsize = 0.25) +
  geom_jitter(position = position_jitter(height = .2, width = .2)) +
  geom_point(test, aes(x=Genotype, y=Period), shape=95, size=15) +
  
  labs(x = "Genotype", 
       y = "Free-running period (hours)") +
  scale_y_continuous(limits=c(21,27),breaks=seq(21, 27, by=1), labels = (seq(21,27,1))) +
  theme_classic() +
  theme(axis.line.x = element_line(color="black", size = .5),
        axis.line.y = element_line(color="black", size = .5))  

p.to.test <- ggplot(test, aes(x=Genotype, y = Period)) +
  geom_point(shape = 95, size = 10) +
  geom_errorbar(aes(ymin=Period-sd, ymax=Period+sd), width=0.5, size = 1) +
  #geom_point(to.plot.data, aes(x=Genotype, y=Period)) +
  labs(x = "Genotype", 
       y = "Free-running period (hours)") +
  scale_y_continuous(limits=c(21,27),breaks=seq(21, 27, by=1), labels = (seq(21,27,1))) +
  theme_classic() +
